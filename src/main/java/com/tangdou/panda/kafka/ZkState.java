...
public class ZkState {
    ...
    private CuratorFramework newCurator(Map conf, KafkaSpoutConfig config) throws Exception {
        ...
        return CuratorFrameworkFactory.newClient(serverPorts, Utils.getInt(conf.get(Config
                .STORM_ZOOKEEPER_SESSION_TIMEOUT)), 15000, new RetryNTimes(
                Utils.getInt(conf.get(Config.STORM_ZOOKEEPER_RETRY_TIMES)), Utils.getInt(conf.get(Config
    ...
    public void writeBytes(String path, byte[] bytes) {
        try {
            if (_curator.checkExists().forPath(path) == null) {
                ...
                ProtectACLCreateModePathAndBytesable<String> createAble =
