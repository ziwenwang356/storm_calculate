...
public class KafkaConsumer {
    ...
    public ByteBufferMessageSet fetchMessages(int partition, long offset) throws IOException {
        ...
        FetchRequest req = new FetchRequestBuilder().clientId(config.clientId).addFetch(topic, partition, offset,
        ...
        try {
            ...
            if (e instanceof ConnectException || e instanceof SocketTimeoutException || e instanceof IOException
                ...
                if (simpleConsumer != null) {
                    ...
                    simpleConsumer = null;
                    throw new KafkaException("Network error when fetching messages: " + host + ":" + port + " , " + e
                            .getMessage(), e);
                }
        ...
        if (fetchResponse.hasError()) {
            ...
            if (leaderBroker != null) {
                LOG.error("fetch data from kafka topic[" + config.topic + "] host[" + leaderBroker.host() + ":" +
                        leaderBroker.port() + "] partition["
    ...
    private SimpleConsumer findLeaderConsumer(int partition) {
        try {
            ...
            consumer = new SimpleConsumer(leaderBroker.host(), leaderBroker.port(), config.socketTimeoutMs, config
    ...
    protected PartitionMetadata findLeader(int partition) {
        ...
        try {
            if (consumer == null) {
                consumer = new SimpleConsumer(brokerHost.getHost(), brokerHost.getPort(), config.socketTimeoutMs,
        ...
        while (i < size && errors < size + 1) {
            ...
            try {
                ...
                if (consumer == null) {
                    consumer = new SimpleConsumer(host.getHost(), host.getPort(), config.socketTimeoutMs, config
                            .socketReceiveBufferBytes,
                            config.clientId);
                }
                ...
                try {
                    ...
                    LOG.error("findLeader error, broker:" + host.toString() + ", will change to next broker index:" +
                            (i + 1) % size, e);
                    if (consumer != null) {
                        consumer.close();
                        consumer = null;
                ...
                LOG.error("Error communicating with Broker:" + host.toString() + ", find Leader for partition:" +
                        partition);
            } finally {
                if (consumer != null) {
                    consumer.close();
